...
function setLanguage(lang) {
    ...
    const uniLogoSrc = lang === 'tr'
        ? 'assets/logos/ozyegin-universitesi.svg'
        : 'assets/logos/ozyegin-university.svg';
    document.querySelectorAll('.logo-uni, .footer-logo-uni').forEach((imgEl) => {
        imgEl.src = uniLogoSrc;
    });
...
function renderProjectGrid(gridId, projectGroup, searchQuery, yearFilter = 'all') {
    ...
    const filteredProjects = projects
        ...
        .sort((a, b) => {
            const yearDiff = getProjectYearLevel(b) - getProjectYearLevel(a);
            if (yearDiff !== 0) return yearDiff;
            return a.student.localeCompare(b.student, 'tr-TR');
        });
